...
``prometheus`` code paths, with NO bench-local reimplementations of the readers:
  ...
  ftw         load_expert_banks(<ftw_dir>)          read the post-repack banks from the
             unified FTW checkpoint -- i.e. prometheus.checkpoint.load_ftw_banks -- after
...
def worker_build(ns):
    ...
    from prometheus.checkpoint.convert import convert_checkpoint
...
def main():
    ...
    from prometheus.gpu_select import single_gpu_arg
    ...
    from prometheus.gpu_select import assign_gpu
    ...
    from prometheus.checkpoint.ftw import is_ftw_checkpoint
