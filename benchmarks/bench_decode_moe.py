...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--aime",
        default=os.environ.get("PROMETHEUS_AIME25_JSONL"),
        help=f"local jsonl instead of downloading {AIME_REPO}; default $PROMETHEUS_AIME25_JSONL",
    )
...
def serve_cmd(args: argparse.Namespace, backend: str, port: int) -> list[str]:
    cmd = [
        sys.executable, "-m", "prometheus.cli", "serve",
        "--model", args.model,
        ...
        for raw in resp:
            ...
            for choice in chunk.get("choices", []):
                ...
                if text:
                    ...
                    pieces.append(text)
    if usage is None:
        sys.exit("[bench] stream ended without a usage chunk; is this a Prometheus server?")
