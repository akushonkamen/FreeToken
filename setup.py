...
def _check_toolchain() -> None:
    path = ROOT / "python" / "prometheus" / "kernel" / "_toolchain.py"
...
def _cuda_runtime_paths() -> tuple[list[str], list[str]]:
    if CUDA_HOME is None:
        raise RuntimeError(
            "CUDA_HOME is required to build prometheus.kernel._pinned_tensor "
...
setup(
    ext_modules=[
        CppExtension(
            ...
            sources=[
                "python/prometheus/kernel/csrc/pinned_tensor.cpp",
            ],
        ...
        CppExtension(
            name="prometheus.kernel._cpu_moe",
            sources=[
                "python/prometheus/kernel/csrc/cpu_moe/cpu_moe_ext.cpp",
            ],
            include_dirs=cuda_include_dirs,
            library_dirs=cuda_library_dirs,
            libraries=["cudart"],
