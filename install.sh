#!/usr/bin/env bash
#
# Prometheus engine installer (Linux, NVIDIA CUDA) — user-facing, wheel-based.
#
# Installs the `prometheus` runtime (the `prom` CLI) and its prebuilt kernel-cache
# wheel into a managed venv, then wires it up so Prometheus Desktop can find it.
...
#   PROMETHEUS_WHEEL               runtime wheel — local path OR URL. Defaults to the
...
#   PROMETHEUS_KERNEL_CACHE_WHEEL  prebuilt kernel-cache wheel — local path OR URL.
#                                 Defaults to $DEFAULT_KERNEL_CACHE_WHEEL_URL; if
#                                 unset and PROMETHEUS_WHEEL is local, the script
#                                 auto-detects a sibling prometheus_kernel_cache-*.whl.
#   PROMETHEUS_HOME                install root (default: ~/.prometheus); venv at $PROMETHEUS_HOME/venv
#   PROMETHEUS_PY_VERSION          python for the venv (default: 3.12 — must match the wheel tag)
#   PROMETHEUS_BIN_DIR             where to symlink `prom` (default: ~/.local/bin)
#   PROMETHEUS_ENV_DIR             environment.d dir (default: ~/.config/environment.d)
#
...
FT_HOME="${PROMETHEUS_HOME:-$HOME/.prometheus}"
VENV="$FT_HOME/venv"
PY_VERSION="${PROMETHEUS_PY_VERSION:-3.12}"
BIN_DIR="${PROMETHEUS_BIN_DIR:-$HOME/.local/bin}"
ENV_DIR="${PROMETHEUS_ENV_DIR:-$HOME/.config/environment.d}"
WHEEL="${PROMETHEUS_WHEEL:-$DEFAULT_WHEEL_URL}"
KERNEL_CACHE_WHEEL="${PROMETHEUS_KERNEL_CACHE_WHEEL:-$DEFAULT_KERNEL_CACHE_WHEEL_URL}"

# --yes / -y (or PROMETHEUS_ASSUME_YES=1): run non-interactively — in particular, bootstrap uv
# without asking. Prometheus Desktop's in-app installer passes --yes (its stdout is piped into a
# modal, so there is no terminal to prompt at).
ASSUME_YES="${PROMETHEUS_ASSUME_YES:-0}"
for _arg in "$@"; do
  case "$_arg" in
    -y|--yes) ASSUME_YES=1 ;;
...
infer_kernel_cache_wheel() {
  ...
  found=""
  count=0
  for candidate in "$wheel_dir"/prometheus_kernel_cache-*.whl "$wheel_dir"/prometheus-kernel-cache-*.whl; do
  ...
  if [ "$count" -gt 1 ]; then
    die "multiple prometheus kernel-cache wheels found next to $WHEEL — set PROMETHEUS_KERNEL_CACHE_WHEEL explicitly."
  fi
  if [ "$count" -eq 1 ]; then
...
build_from_repo_if_needed() {
  ...
  # staleness to defend against. PROMETHEUS_BUILD_NO_STAMP=0 forces a stamp anyway.
  PROMETHEUS_BUILD_OUT_DIR="$out_dir" PROMETHEUS_BUILD_NO_STAMP="${PROMETHEUS_BUILD_NO_STAMP:-1}" bash "$builder"

  rt="$(ls -t "$out_dir"/prometheus-*.whl 2>/dev/null | head -1)" || true
  [ -n "$rt" ] || die "build finished but no prometheus-*.whl found in $out_dir"
  WHEEL="$rt"
  say "built runtime wheel: $WHEEL"
  if [ -z "$KERNEL_CACHE_WHEEL" ]; then
    kc="$(ls -t "$out_dir"/prometheus_kernel_cache-*.whl 2>/dev/null | head -1)" || true
    if [ -n "$kc" ]; then
...
# The Prometheus Desktop engine bundle ships the wheels in ./dist next to this script.
find_bundled_wheel() {
  [ -z "$WHEEL" ] || return 0
  ...
  [ -d "$dist" ] || return 0
  rt="$(ls -t "$dist"/prometheus-*.whl 2>/dev/null | grep -Ev 'prometheus[_-]kernel[_-]cache-' | head -1)" || true
...
[ -n "$WHEEL" ] || die "no runtime wheel to install — set PROMETHEUS_WHEEL to a local path or URL, or run install.sh from a source checkout to build one."
infer_kernel_cache_wheel
[ -n "$KERNEL_CACHE_WHEEL" ] || die "no kernel-cache wheel to install — set PROMETHEUS_KERNEL_CACHE_WHEEL to a local path or URL."
...
"$UV" pip install --python "$VENV" \
  --refresh-package prometheus --refresh-package prometheus-kernel-cache \
  "${CU_INDEX_ARGS[@]}" "${INSTALL_WHEELS[@]}"

FT_BIN="$VENV/bin/prom"
[ -x "$FT_BIN" ] || die "install finished but $FT_BIN is missing."

# --- 4. Wire up for PATH + Prometheus Desktop -------------------------------
mkdir -p "$BIN_DIR"
ln -sf "$FT_BIN" "$BIN_DIR/prom"
say "symlinked $BIN_DIR/prom -> $FT_BIN"

mkdir -p "$ENV_DIR"
printf 'PROMETHEUS_FT_BIN=%s\n' "$FT_BIN" > "$ENV_DIR/50-prometheus.conf"
say "wrote $ENV_DIR/50-prometheus.conf (PROMETHEUS_FT_BIN) — GUI picks it up after next login"

# --- 5. Self-check ---------------------------------------------------------
if "$FT_BIN" --help >/dev/null 2>&1; then
  say "self-check: \`prom --help\` OK"
else
  warn "self-check: \`prom --help\` returned non-zero — inspect with: $FT_BIN --help"
fi

cat <<EOF

${C_GREEN}Prometheus engine installed.${C_RESET}

  prom binary        $FT_BIN
  on PATH as       $BIN_DIR/prom   (ensure $BIN_DIR is on PATH)
  Desktop env      PROMETHEUS_FT_BIN via environment.d (re-login to apply)

Run in this shell without re-login:
  export PROMETHEUS_FT_BIN="$FT_BIN"
  prom serve --model <path> --port 1919
