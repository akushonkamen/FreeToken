...
grabs a helper from ``prometheus.server`` or ``prometheus.utils`` would silently pull torch and only
...
DAEMON_MODULES = [
    "prometheus.daemon",
    "prometheus.daemon.version",
    "prometheus.daemon.accounting",
    "prometheus.daemon.logfmt",
    "prometheus.daemon.logring",
    "prometheus.daemon.osproc",
    "prometheus.daemon.pidfile",
    "prometheus.daemon.metrics",
    "prometheus.daemon.proxy",
    "prometheus.daemon.tailer",
    "prometheus.daemon.serve_manager",
    "prometheus.daemon.checkpoint",
    "prometheus.daemon.app",
    "prometheus.daemon.client",
    "prometheus.daemon.server",
]
...
try:
    ...
    from prometheus.daemon.app import build_app
    ...
    build_app(
        manager=_Mgr(),
        ring=importlib.import_module("prometheus.daemon.logring").LogRing(),
        probe=None,
