...
def _patch_tp(monkeypatch) -> None:
    from prometheus.distributed.info import DistributedInfo

    monkeypatch.setattr(
        "prometheus.kvcache.hybrid_swa_pool.get_tp_info",
...
def test_create_kvcache_pool_uses_hybrid_swa_cache(monkeypatch):
    from prometheus.kvcache import create_kvcache_pool
    from prometheus.kvcache.hybrid_swa_pool import HybridSWAKVCache
...
def test_adjust_config_resolves_swa_cache_type():
    ...
    from prometheus.scheduler.config import SchedulerConfig
