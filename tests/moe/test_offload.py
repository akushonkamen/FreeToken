...
def _make_layer_and_cache():
    from prometheus.layers.moe import OffloadMoELayer
...
def test_dummy_expert_sources_use_moe_layer_count(monkeypatch):
    ...
    import prometheus.models.weight as weight
...
def test_offload_moe_layer_prefill_forward_uses_single_layer_cache_view(monkeypatch):
    ...
    monkeypatch.setattr(
        "prometheus.layers.moe.fused_topk",
    ...
    monkeypatch.setattr("prometheus.layers.moe.fused_experts_impl", fake_fused)
...
def test_offload_moe_layer_prefill_overlap_prefetches_layers_into_two_buffers(monkeypatch):
    ...
    monkeypatch.setattr("prometheus.kernel.fast_index_copy_jit", unexpected_fast_index_copy)
...
def test_offload_moe_layer_decode_forward_uses_remapped_slot_ids(monkeypatch):
    ...
    monkeypatch.setattr("prometheus.layers.moe.fused_experts_decode_impl", fake_fused_decode)
...
def test_adjust_config_converts_moe_cache_rate_to_cache_size():
    ...
    from prometheus.distributed import DistributedInfo
    from prometheus.engine.config import EngineConfig
    from prometheus.engine.engine import _adjust_config

    config = EngineConfig(
        model_path="/tmp/prometheus-test-model",
...
def test_graph_capture_reuses_warm_offload_cache_before_capture(monkeypatch):
    import prometheus.core as core
    from prometheus.core import Context, Req, get_global_ctx
    from prometheus.engine.graph import GraphRunner
    ...
    monkeypatch.setattr("prometheus.engine.graph.get_free_memory", lambda device: 1024)
...
def test_offload_cache_validate_rebuild_enforces_marlin_cap_and_floor():
    ...
    from prometheus.moe.offload_cache import MARLIN_MAX_CACHE_SIZE, OffloadMoeCache
