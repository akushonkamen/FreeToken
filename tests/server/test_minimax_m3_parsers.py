...
from prometheus.server.reasoning_parser import MiniMaxM3ReasoningParser, ReasoningParser
...
def test_detect_and_parse_unknown_tool_follows_forwarding_policy():
    # Prometheus forwards unknown tool names by default (FORWARD_UNKNOWN_TOOLS);
...
def test_streaming_truncated_call_suppressed_and_recovered():
    ...
    from prometheus.server.function_call_parser import FunctionCallParser
...
def test_think_gears(gear, mode):
    ...
    from prometheus.tokenizer.effort import EffortProfile, probe_thinking_profile
