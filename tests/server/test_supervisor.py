...
from prometheus.utils import progress
from prometheus.server.supervisor import BackendHandle, LoadProgress, drain_ready, phase_slug
...
def test_drain_ready_detects_worker_death_during_load():
    ...
    from prometheus.server.supervisor import WorkerDied
...
def test_supervisor_reports_the_worker_error_reason_via_on_failure():
    ...
    handle = BackendHandle(ack_queue=q, processes=[DeadProc()], expected_acks=1)
    seen: dict = {}
    from prometheus.server.supervisor import run_backend_supervisor
