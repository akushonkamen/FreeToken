...
from prometheus.server.function_call_parser import (
    Function,
    FunctionCallParser,
    MuseGlimmerDetector,
    Tool,
)
from prometheus.server.reasoning_parser import (
    ATEM_START,
...
def test_unknown_tool_follows_forwarding_policy():
    import prometheus.server.function_call_parser as fcp
...
def test_reasoning_strength_kwarg_does_not_swallow_the_thinking_toggle():
    ...
    from prometheus.server.model_meta import effort_toggle_kwargs
...
def test_mixed_quant_groups_rejected_in_any_order():
    ...
    for order in (("group_0", "group_1"), ("group_1", "group_0")):
        ...
        from prometheus.models.muse_glimmer.config import parse_config as pc
...
def test_auto_selection_picks_muse_glimmer():
    ...
    from prometheus.server.args import parse_args
    ...
    with patch("prometheus.utils.cached_load_hf_config", lambda _p: _Config()):
...
def test_probed_muse_gears_and_effort_vocabulary():
    ...
    from prometheus.server.model_meta import derive_think_gears
    from prometheus.tokenizer.effort import effective_efforts, quantize_effort
    from prometheus.tokenizer.tokenize import TokenizeManager
