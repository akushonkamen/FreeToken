...
def test_triton_backend_passes_attention_sinks_to_paged_kernel(monkeypatch):
    from prometheus.attention import AttentionSpec
    from prometheus.attention.triton import TritonAttentionBackend, TritonMetadata
    ...
    monkeypatch.setattr(
        "prometheus.attention.triton.get_global_ctx",
    ...
    monkeypatch.setattr("prometheus.kernel.triton.attention.paged_attention", fake_paged_attention)
...
def test_paged_triton_attention_matches_reference(head_dim: int, sliding_window: int | None):
    from prometheus.kernel.triton.attention import paged_attention
...
def test_decode_triton_attention_matches_reference(
    ...
    from prometheus.kernel.triton.attention import decode_paged_attention
...
def test_extend_triton_attention_matches_reference(
    ...
    from prometheus.kernel.triton.attention import extend_paged_attention
...
def test_select_extend_tile_is_shared_memory_aware(head_dim, smem_optin, expected):
    ...
    from prometheus.kernel.triton.attention import _select_extend_tile
...
def test_triton_backend_stores_kv_and_matches_reference(monkeypatch):
    ...
    monkeypatch.setattr("prometheus.attention.triton.get_global_ctx", lambda: ctx)
