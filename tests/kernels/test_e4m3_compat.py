...
2. Every affected wrapper produces bit-identical output under
   ``PROMETHEUS_FORCE_E4M3_EMU=1`` (run in a subprocess with a fresh
...
class TestPrimitives:
    def test_decode_f32_bitexact(self):
        ...
        from prometheus.kernel.triton.e4m3_compat import e4m3_u8_to_f32
    ...
    def test_decode_f16_x128_bitexact(self):
        import triton
        import triton.language as tl
        from prometheus.kernel.triton.e4m3_compat import e4m3_u8_to_f16_x128
    ...
    def test_round_to_grid_bitexact(self):
        import triton
        import triton.language as tl
        from prometheus.kernel.triton.e4m3_compat import round_e4m3
...
def _emit_all(path: str) -> None:
    from prometheus.kernel.triton.e4m3_compat import e4m3_native
    from prometheus.kernel.triton.dsv4.fp8_linear import (
    ...
    from prometheus.kernel.triton.fp8_pertensor_linear import fp8_pertensor_linear
    from prometheus.kernel.triton.fp8_block_linear import (
        block_fp8_linear, per_token_group_quant_fp8,
    )
    from prometheus.kernel.triton.fp8_blockscale_moe import (
    ...
    from prometheus.moe.fused_nvfp4 import (
...
def _child_env(tmp_path, **extra) -> dict:
    ...
    env = os.environ.copy()
    pkg_root = os.path.dirname(os.path.dirname(prometheus.__file__))
...
def test_forced_emu_matches_native(tmp_path):
    ...
    r = subprocess.run(
        ...
        env=_child_env(tmp_path, PROMETHEUS_FORCE_E4M3_EMU="1"),
...
def test_compile_gate_foreign_arch(arch, tmp_path):
    r = subprocess.run(
        ...
        env=_child_env(tmp_path, PROMETHEUS_FORCE_E4M3_EMU=""),
        capture_output=True, text=True, timeout=1200,
    )
...
def _gate_main(arch: int) -> None:
    ...
    import prometheus.kernel.backend as backend
