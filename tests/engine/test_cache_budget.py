...
from prometheus.engine.cache_budget import expert_bytes_per_slot, plan_cache_budget, resolve_moe_cache_auto
...
def test_engine_resolve_auto_moe_cache_size_maps_kwargs():
    ...
    from prometheus.engine.engine import Engine
    from prometheus.models.config import KVCacheGroupSpec
...
def test_guard_passes_when_size_covers_one_expert_per_layer():
    from prometheus.engine.engine import _require_offload_cache_size
...
def test_adjust_config_defaults_moe_cache_auto_for_auto_resolved_offload_backend():
    """Bare `prom serve <FTW MoE checkpoint>`: no --moe-backend, no --moe-cache-* flags at all.
    ...
    # decide: a bare "auto" consults ~/.cache/prometheus/benchbw.json, so a box that has run
    # `prom bench bw` resolves nvfp4 experts to hybrid instead. Assert the family, not the member.
...
def test_page_table_width_covers_whole_trailing_pages():
    ...
    from prometheus.engine.engine import _page_table_width
