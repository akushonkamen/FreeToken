...
Directories mirror the `python/prometheus/` subsystem a test primarily exercises —
...
|--------------|----------------------|
| `daemon/`    | `prometheus.daemon` — torch-free supervisor (import safety, serve manager) |
| `server/`    | `prometheus.server` — OpenAI/Anthropic/Responses APIs, streaming, parsers, accounting, maintenance state |
| `scheduler/` | `prometheus.scheduler` — chunked prefill, cache manager, commit/window locking, status reporting, KV usage, cost accounting |
| `kvcache/`   | `prometheus.kvcache` — paged pools (incl. DSV4's), rebuild, cache-cost accounting; the three prefix caches live in `kvcache/radix/` behind a shared reference model (see its README) |
| `tokenizer/` | `prometheus.tokenizer` — tokenize/detokenize request plumbing, thinking-mode resolution |
| `engine/`    | `prometheus.engine` — cache budget planning, config resolution, backend gating |
| `kernels/`   | `prometheus.kernel` / `prometheus.layers` — attention, rope, fused ops, JIT cache, pinned memory |
| `moe/`       | `prometheus.moe` — offload cache, CPU/GPU expert kernels, quantized backends (fp8/nvfp4/mxfp4/q4_0) |
| `models/`    | `prometheus.models` — the registry, and the loading machinery every model shares (sharding, qkv/expert merge, streaming layers into banks) |
...
| env var                    | used by |
|----------------------------|---------|
| `PROMETHEUS_TEST_MODEL`     | `e2e/test_aime.py` — local model directory |
| `PROMETHEUS_AIME_SERIES`    | `e2e/test_aime.py` — `aime24` \| `aime25` \| `aime26` (default `aime25`) |
| `PROMETHEUS_AIME{24,25,26}_JSONL` | `e2e/test_aime.py` — jsonl for the selected series |
| `PROMETHEUS_AIME_REQ`       | `e2e/test_aime.py` — 1-based problem id, comma list (`1,3,7`), or `all` (default `1`) |
| `PROMETHEUS_AIME_MAX_TOKENS`| `e2e/test_aime.py` — per-sample token budget (default `16384`) |
| `PROMETHEUS_AIME_SAMPLES`   | `e2e/test_aime.py` — pass@N sample count when sampling (default `3`) |
| `PROMETHEUS_AIME_MIN_FREE_GIB` | `e2e/test_aime.py` — required free GPU memory (default `70`) |
| `PROMETHEUS_TEST_MOE_CACHE_SIZE` | `e2e/test_aime.py` — >0 switches to the offload MoE backend with this cache size |
| `PROMETHEUS_TEST_MEM_RATIO` | `e2e/test_aime.py` — offload-mode memory_ratio (default `0.9`) |
| `PROMETHEUS_REBUILD_TEST_MODEL` | `e2e/test_cache_rebuild.py` — a SMALL local model dir; boots a real server (falls back to `PROMETHEUS_TEST_MODEL`) |
| `PROMETHEUS_GEMMA4_GGUF_GLOB` | `models/test_gemma4_gguf_rope.py` — glob matching a local gemma-4 GGUF file |
...
point `PROMETHEUS_TEST_MODEL` at the fp8 dir and set `PROMETHEUS_TEST_MOE_CACHE_SIZE=8192`;
`PROMETHEUS_AIME_REQ=2 PROMETHEUS_AIME_MAX_TOKENS=32768` is the offload recipe (req 2 with
a 32k budget).
...
```bash
PROMETHEUS_REBUILD_TEST_MODEL=<small model dir> uv run pytest tests/e2e/test_cache_rebuild.py
```
