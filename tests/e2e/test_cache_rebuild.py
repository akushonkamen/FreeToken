...
Boots ``prom serve`` on a small checkpoint and drives POST /v1/cache/rebuild for real: the whole
...
Gated behind ``needs_weights``:
  ...
  PROMETHEUS_REBUILD_TEST_MODEL  small local model dir (falls back to PROMETHEUS_TEST_MODEL)
  PROMETHEUS_REBUILD_MIN_FREE_GIB  free-GPU-memory gate (default 20)
  PROMETHEUS_REBUILD_BOOT_TIMEOUT  seconds to wait for "serving" (default 300)
"""
...
def _model_dir() -> Path | None:
    value = os.environ.get("PROMETHEUS_REBUILD_TEST_MODEL") or os.environ.get("PROMETHEUS_TEST_MODEL")
...
def test_cache_rebuild_resizes_a_live_engine_and_keeps_serving(tmp_path):
    ...
    if model_dir is None:
        pytest.skip("set PROMETHEUS_REBUILD_TEST_MODEL to a small local model directory")
    if not model_dir.is_dir():
        pytest.skip(f"model is not downloaded: {model_dir}")

    min_free = float(os.environ.get("PROMETHEUS_REBUILD_MIN_FREE_GIB", "20"))
    ...
    try:
        boot_timeout = float(os.environ.get("PROMETHEUS_REBUILD_BOOT_TIMEOUT", "300"))
