...
Gated behind ``needs_weights``: set ``PROMETHEUS_TEST_MODEL`` to the model dir and
the series jsonl env var (see below), and run on an idle GPU.

Env knobs:
  PROMETHEUS_TEST_MODEL          local model directory (required)
  PROMETHEUS_AIME_SERIES         aime24 | aime25 | aime26 (default aime25)
  PROMETHEUS_AIME{24,25,26}_JSONL  path to the chosen series' jsonl (required)
  PROMETHEUS_AIME_REQ            1-based problem id, comma list, or 'all' (default '1')
  PROMETHEUS_AIME_MAX_TOKENS     per-sample token budget (default 16384)
  PROMETHEUS_AIME_SAMPLES        pass@N sample count when sampling (default 3)
  PROMETHEUS_AIME_MIN_FREE_GIB   free-GPU-memory gate (default 70; raise for a big resident model)
  PROMETHEUS_TEST_MOE_CACHE_SIZE >0 switches to the offload MoE backend (fp8/GLM/MiniMax)
  PROMETHEUS_TEST_MEM_RATIO      offload memory ratio (default 0.9)

fp8 / offload recipe: point PROMETHEUS_TEST_MODEL at the fp8 checkpoint dir and set
PROMETHEUS_TEST_MOE_CACHE_SIZE (e.g. 8192).
"""
...
from prometheus.core import SamplingParams
from prometheus.llm import LLM
...
AIME_SERIES_JSONL_ENV = {
    "aime24": "PROMETHEUS_AIME24_JSONL",
    "aime25": "PROMETHEUS_AIME25_JSONL",
    "aime26": "PROMETHEUS_AIME26_JSONL",
}
...
def series_jsonl_env(series: str) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"unknown PROMETHEUS_AIME_SERIES {series!r}; want one of {sorted(AIME_SERIES_JSONL_ENV)}"
        )


def parse_req_selection(value: str, n_rows: int) -> list[int]:
    """Parse PROMETHEUS_AIME_REQ into 0-based row indices into a jsonl of ``n_rows`` problems.
...
def max_tokens() -> int:
    return int(os.environ.get("PROMETHEUS_AIME_MAX_TOKENS", "16384"))
...
def recommended_sampling(model_dir: Path) -> tuple[SamplingParams, int]:
    ...
    n_samples = int(os.environ.get("PROMETHEUS_AIME_SAMPLES", "3"))
    return sp, n_samples


def build_llm(model_path: Path) -> LLM:
    """Resident by default; set PROMETHEUS_TEST_MOE_CACHE_SIZE>0 to run an offload model
    (fp8 Qwen3.5-MoE, GLM-4.7, MiniMax, ... -- routed experts that do not fit in HBM)."""
    kwargs = dict(
        model_path=str(model_path),
        dtype=torch.bfloat16,
        # "auto" is what `prom serve` resolves, so the gate exercises the backend a model
    ...
    cache_size = int(os.environ.get("PROMETHEUS_TEST_MOE_CACHE_SIZE", "0"))
    if cache_size > 0:
        kwargs.update(
            moe_backend="offload",
            moe_cache_size=cache_size,
            moe_cache_policy="lru",
            memory_ratio=float(os.environ.get("PROMETHEUS_TEST_MEM_RATIO", "0.9")),
...
def test_aime():
    model_path = _optional_path("PROMETHEUS_TEST_MODEL")
    if model_path is None:
        pytest.skip("set PROMETHEUS_TEST_MODEL to a local model directory")
    if not model_path.is_dir():
        pytest.skip(f"model is not downloaded: {model_path}")

    series = os.environ.get("PROMETHEUS_AIME_SERIES", "aime25").strip().lower()
    try:
