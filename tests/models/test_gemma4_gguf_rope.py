...
GGUF_GLOB = os.environ.get("PROMETHEUS_GEMMA4_GGUF_GLOB", "")
...
@pytest.mark.skipif(not glob.glob(GGUF_GLOB), reason="PROMETHEUS_GEMMA4_GGUF_GLOB not set to a local gemma4 GGUF")
def test_full_rotary_dim_recovered_from_rope_freqs():
    from prometheus.models.gemma4.gguf import _full_rotary_dim
...
def test_proportional_rope_matches_llama_cpp_rope_freqs_division():
    ...
    Prometheus's "proportional" branch must produce the same table."""
