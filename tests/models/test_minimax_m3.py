...
from prometheus.attention.base import AttnType
from prometheus.models.minimax_m3.config import parse_config
...
def test_parse_config_native_shape_matches_raw(monkeypatch):
    ...
    monkeypatch.delenv("PROMETHEUS_M3_MAX_LAYERS", raising=False)
    monkeypatch.delenv("PROMETHEUS_M3_SPARSE", raising=False)
...
def test_pool_family_and_kv_cost():
    ...
    from prometheus.kvcache.bsa_pool import BSAKVCache
...
def test_layer_cap_env(monkeypatch):
    monkeypatch.setenv("PROMETHEUS_M3_MAX_LAYERS", "5")
    cfg = parse_config(_hf_config())
...
def test_sparse_ablation_env(monkeypatch):
    monkeypatch.setenv("PROMETHEUS_M3_SPARSE", "0")
    cfg = parse_config(_hf_config())
    (spec,) = cfg.kv_cache_group_specs()
...
def test_mxfp8_ablation_env(monkeypatch):
    monkeypatch.setenv("PROMETHEUS_M3_ATTN_MXFP8", "0")
    monkeypatch.setenv("PROMETHEUS_M3_MLP_MXFP8", "0")
    cfg = parse_config(_hf_config())
    assert cfg.attn_quant == "none" and cfg.dense_quant == "none"


def test_registry_resolves_both_architectures():
    from prometheus.models.register import get_model_spec

    for arch in ("MiniMaxM3SparseForConditionalGeneration", "MiniMaxM3SparseForCausalLM"):
        spec = get_model_spec(arch)
        assert spec.module == "prometheus.models.minimax_m3"


def test_auto_backend_resolution():
    from prometheus.attention import attention_backend_info
    from prometheus.engine.engine import _required_attn_types, _resolve_auto_attention_backend
...
def test_nvfp4_backend_restricted_to_triton_for_swigluoai():
    from prometheus.moe.nvfp4_backends import select_nvfp4_backend
...
def test_expert_source_spec_layer_to_bank():
    from prometheus.models.minimax_m3.weight import _EXPERT_KEY_RE, _NVFP4_SOURCE_SPEC
