...
#                      uv's package cache across builds (default: ~/.cache/prometheus-ci)
...
#   PROMETHEUS_BUILD_NO_STAMP / _RELEASE / _DEV_STAMP / _STRIP and
#   PROMETHEUS_KERNEL_CACHE_* are forwarded into the container. Other
#   PROMETHEUS_BUILD_* vars are NOT: _CLEAN is set by this script per matrix
...
if [[ -z "${FT_IN_CONTAINER:-}" ]]; then
  ...
  CACHE_DIR="${FT_CI_CACHE_DIR:-$HOME/.cache/prometheus-ci}"
  ...
  exec docker run --rm \
    ...
    -e FT_HOST_UID="$(id -u)" \
    -e FT_HOST_GID="$(id -g)" \
    -e PROMETHEUS_BUILD_NO_STAMP="${PROMETHEUS_BUILD_NO_STAMP:-}" \
    -e PROMETHEUS_BUILD_RELEASE="${PROMETHEUS_BUILD_RELEASE:-}" \
    -e PROMETHEUS_BUILD_DEV_STAMP="${PROMETHEUS_BUILD_DEV_STAMP:-}" \
    -e PROMETHEUS_BUILD_STRIP="${PROMETHEUS_BUILD_STRIP:-}" \
    -e PROMETHEUS_KERNEL_CACHE_SPECS="${PROMETHEUS_KERNEL_CACHE_SPECS:-}" \
    -e PROMETHEUS_KERNEL_CACHE_VERBOSE="${PROMETHEUS_KERNEL_CACHE_VERBOSE:-}" \
...
restore_ownership() {
  ...
  chown "$FT_HOST_UID:$FT_HOST_GID" \
    /workspace/python/prometheus/version.py /workspace/.git/index 2>/dev/null || true
}
...
export PROMETHEUS_BUILD_OUT_DIR=/ci-out
...
rm -f /ci-out/prometheus-*.whl /ci-out/prometheus_kernel_cache-*.whl /ci-out/prometheus-kernel-cache-*.whl
export PROMETHEUS_BUILD_CLEAN=0
...
for cptag in $MATRIX; do
  ...
  export PROMETHEUS_BUILD_PYTHON="$VENV/bin/python"
  # The kernel-cache wheel is py3-none: build it once, with the first interpreter.
  export PROMETHEUS_BUILD_SKIP_KERNEL_CACHE="$((1 - first))"
  first=0
...
# leaves the kernel-cache wheel alone: prometheus_* does not match prometheus-*.
case " 1 true yes on " in *" $(printf '%s' "$RETAG" | tr '[:upper:]' '[:lower:]') "*)
  ...
  found=0
  for whl in /ci-out/prometheus-*linux_x86_64.whl; do
    [[ -e "$whl" ]] || continue
