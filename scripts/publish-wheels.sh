#!/usr/bin/env bash
#
# Publish built engine wheels to the Prometheus-Web rolling `beta` release.
#
# Usage:
...
#
# Environment:
#   PROMETHEUS_WEB_REPO   target repo  (default: prometheus/prometheus-web)
#   PROMETHEUS_WEB_TAG    release tag  (default: beta)
set -euo pipefail

REPO="${PROMETHEUS_WEB_REPO:-prometheus/prometheus-web}"
TAG="${PROMETHEUS_WEB_TAG:-beta}"
DIST="${1:-dist}"
...
shopt -s nullglob
wheels=("$DIST"/prometheus-*.whl "$DIST"/prometheus_kernel_cache-*.whl)
[ "${#wheels[@]}" -gt 0 ] || die "no prometheus wheels in $DIST"
...
while IFS= read -r p; do
  while IFS= read -r name; do
    case "$name" in
      prometheus-*"$p"*.whl | prometheus_kernel_cache-*"$p"*.whl)
