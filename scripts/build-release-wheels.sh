#!/usr/bin/env bash
#
# Build the Prometheus runtime wheel and the matching prebuilt kernel-cache wheel.
#
# Common use:
#   scripts/build-release-wheels.sh
#
# Useful environment:
#   PROMETHEUS_BUILD_OUT_DIR       output directory (default: ./dist)
#   PROMETHEUS_BUILD_PYTHON        python used by uv build (default: ./.venv/bin/python)
#   PROMETHEUS_BUILD_NO_ISOLATION  pass --no-build-isolation when true (default: 1)
#   PROMETHEUS_BUILD_CLEAN         remove old Prometheus wheels from out dir first (default: 1)
#   PROMETHEUS_BUILD_KEEP_TEMP     keep setuptools/tvm build leftovers when true (default: 0)
#   PROMETHEUS_BUILD_STRIP         strip debug symbols from the runtime wheel's .so (default: 1)
#   PROMETHEUS_BUILD_NO_STAMP      skip the +g<sha> commit stamp -- dev builds only (default: 0)
#   PROMETHEUS_BUILD_RELEASE       tagged-release mode: no stamp, and HEAD must be at the
#                                 tag v<version.py> exactly (default: 0)
#   PROMETHEUS_BUILD_DEV_STAMP     stamp .dev<N> instead of +g<sha> -- PEP 440-legal without
#                                 a local segment, so TestPyPI rehearsals get a unique,
#                                 uploadable version per run (default: unset)
#   PROMETHEUS_BUILD_SKIP_KERNEL_CACHE  build only the runtime wheel -- for the 2nd..Nth
#                                 interpreter of a matrix build; the kernel cache is
#                                 py3-none and only needs building once (default: 0)
#   PROMETHEUS_KERNEL_CACHE_SPECS  optional comma-separated subset of kernel spec names
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
cd "$ROOT"

OUT_DIR="${PROMETHEUS_BUILD_OUT_DIR:-$ROOT/dist}"
PYTHON_BIN="${PROMETHEUS_BUILD_PYTHON:-$ROOT/.venv/bin/python}"
NO_BUILD_ISOLATION="${PROMETHEUS_BUILD_NO_ISOLATION:-1}"
CLEAN="${PROMETHEUS_BUILD_CLEAN:-1}"
KEEP_TEMP="${PROMETHEUS_BUILD_KEEP_TEMP:-0}"
STRIP="${PROMETHEUS_BUILD_STRIP:-1}"
NO_STAMP="${PROMETHEUS_BUILD_NO_STAMP:-0}"
RELEASE="${PROMETHEUS_BUILD_RELEASE:-0}"
DEV_STAMP="${PROMETHEUS_BUILD_DEV_STAMP:-}"
SKIP_KERNEL_CACHE="${PROMETHEUS_BUILD_SKIP_KERNEL_CACHE:-0}"
...
warn_arch_override() {
  local var
  for var in TVM_FFI_CUDA_ARCH_LIST PROMETHEUS_KERNEL_CACHE_ARCHES; do
    if [[ -n "${!var:-}" ]]; then
      warn "############################################################"
      warn "$var='${!var}' is set in this shell and OVERRIDES the"
      warn "default multi-arch list (8.6 8.9 9.0 10.0 12.0, see"
      warn "prometheus-kernel-cache/build_backend.py). The kernel-cache"
...
cleanup_generated() {
  ...
  rm -rf \
    "$ROOT/build" \
    "$ROOT/python/prometheus.egg-info" \
    "$ROOT/prometheus-kernel-cache/build" \
    "$ROOT/prometheus-kernel-cache/prometheus_kernel_cache.egg-info" \
    "$ROOT/prometheus-kernel-cache/__pycache__" \
    "$ROOT/prometheus-kernel-cache/prometheus_kernel_cache/__pycache__" \
    "$ROOT/prometheus-kernel-cache/prometheus_kernel_cache/_build_meta.py" \
    "$ROOT/prometheus-kernel-cache/prometheus_kernel_cache/jit_cache"
}
...
VERSION_FILE="$ROOT/python/prometheus/version.py"
STAMPED=0
restore_version() {
  if [[ "$STAMPED" == 1 ]]; then
    git -C "$ROOT" checkout --quiet -- "python/prometheus/version.py" \
      || warn "could not restore python/prometheus/version.py -- it is still stamped; run: git checkout -- python/prometheus/version.py"
  fi
}
stamp_version() {
  if [[ -n "$DEV_STAMP" ]] && { enabled "$RELEASE" || enabled "$NO_STAMP"; }; then
    die "PROMETHEUS_BUILD_DEV_STAMP cannot be combined with RELEASE or NO_STAMP modes"
  fi
  ...
  if enabled "$RELEASE"; then
    enabled "$NO_STAMP" \
      && die "PROMETHEUS_BUILD_RELEASE and PROMETHEUS_BUILD_NO_STAMP are mutually exclusive"
    ...
    tag="$(git -C "$ROOT" describe --exact-match --tags HEAD 2>/dev/null)" \
      || die "PROMETHEUS_BUILD_RELEASE: HEAD is not at a tag (expected tag v$version)."
    [[ "$tag" == "v$version" ]] \
      || die "PROMETHEUS_BUILD_RELEASE: HEAD tag is '$tag' but version.py says '$version' (expected tag v$version)."
    say "release build: $version (tag $tag)"
    return 0
  fi
  if enabled "$NO_STAMP"; then
    warn "PROMETHEUS_BUILD_NO_STAMP is set -- building UNSTAMPED dev wheels (do not release)"
    return 0
  fi
  git -C "$ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
    || die "not a git checkout -- cannot stamp the build commit. PROMETHEUS_BUILD_NO_STAMP=1 builds unstamped."
  ...
  if [[ "$(git -C "$ROOT" status --porcelain)" == " M python/prometheus/version.py" ]] \
  ...
  if [[ -n "$(git -C "$ROOT" status --porcelain)" ]]; then
    die "working tree is not clean -- a stamped wheel would lie about its commit. Commit/stash (and clean untracked files) first, or set PROMETHEUS_BUILD_NO_STAMP=1 for an unstamped dev build."
  fi
  local suffix version
  if [[ -n "$DEV_STAMP" ]]; then
    [[ "$DEV_STAMP" =~ ^[0-9]+$ ]] || die "PROMETHEUS_BUILD_DEV_STAMP must be a plain number (got '$DEV_STAMP')"
...
if enabled "$CLEAN"; then
  rm -f \
    "$OUT_DIR"/prometheus-*.whl \
    "$OUT_DIR"/prometheus_kernel_cache-*.whl \
    "$OUT_DIR"/prometheus-kernel-cache-*.whl
fi
...
stamp_version
warn_arch_override
say "building prometheus runtime wheel"
uv "${BUILD_ARGS[@]}" .

# Select the wheel THIS interpreter just built by its cp tag -- a matrix build runs
# this script once per interpreter with CLEAN=0, so several prometheus-*.whl coexist.
cptag="$("$PYTHON_BIN" -c 'import sys; print(f"cp{sys.version_info[0]}{sys.version_info[1]}")')"
rt_whl="$(find "$OUT_DIR" -maxdepth 1 -name "prometheus-*-${cptag}-*.whl" -printf '%T@ %p\n' \
  | sort -n | tail -1 | cut -d' ' -f2-)"
[ -n "$rt_whl" ] || die "runtime wheel for $cptag not found in $OUT_DIR"
strip_wheel "$rt_whl"

if enabled "$SKIP_KERNEL_CACHE"; then
  say "skipping prometheus-kernel-cache wheel (PROMETHEUS_BUILD_SKIP_KERNEL_CACHE)"
else
  say "building prometheus-kernel-cache wheel"
  warn_arch_override
  export PROMETHEUS_KERNEL_CACHE_VERBOSE="${PROMETHEUS_KERNEL_CACHE_VERBOSE:-1}"
  export PROMETHEUS_KERNEL_CACHE_BUILD_DIR="${PROMETHEUS_KERNEL_CACHE_BUILD_DIR:-$ROOT/build/prometheus-kernel-cache}"
  uv "${BUILD_ARGS[@]}" prometheus-kernel-cache
fi

say "wheels written to $OUT_DIR"
find "$OUT_DIR" -maxdepth 1 -type f \
  \( -name 'prometheus-*.whl' -o -name 'prometheus_kernel_cache-*.whl' \) \
  -printf '  %p\n' | sort
