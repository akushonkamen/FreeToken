...
def _detach_process_group() -> None:
    ...
    ``prom serve`` keeps the default — uvicorn owns ^C there, and the group-wide delivery is part
    of how it stops."""
    try:
        os.setpgrp()
...
def _run_tokenize_worker(detach: bool, **kwargs) -> None:
    ...
    from prometheus.tokenizer import tokenize_worker
...
def _run_scheduler(args: ServerArgs, ack_queue: mp.Queue[str]) -> None:
    ...
    from prometheus.gpu_select import set_assigned_gpu
    ...
    if args.tp_info.is_primary():
        from prometheus.utils.progress import set_progress_sink
    ...
    with torch.inference_mode():
        ...
        if args.tp_info.is_primary():
            ...
            try:
                from prometheus.kvcache.cache_status import compute_cache_status_meta
...
def launch_server(
    ...
    if server_args.gpu:
        ...
        try:
            ...
            raise SystemExit(f"{prog or 'prom serve'}: error: {exc}") from exc
        logger.info(
    ...
    def start_subprocess() -> "BackendHandle":
        ...
        for i in range(world_size):
            ...
            p = mp.Process(
                ...
                daemon=False,
                name=f"prometheus-TP{i}-scheduler",
            )
            p.start()
            processes.append(p)
