...
logger = init_logger(__name__, "InferenceEngine")
    ...
    def frontend_tokenizer(self) -> Any:
        ...
        with self._frontend_tokenizer_lock:
            if self._frontend_tokenizer is None:
                from prometheus.tokenizer.tokenize import TokenizeManager
                from prometheus.utils import load_tokenizer
...
app = FastAPI(title="Prometheus API Server", version=__version__, lifespan=lifespan)
...
async def dispatch_rebuild(
    ...
    *,
    ...
    through ``POST /v1/cache/rebuild`` (``prom ctl cache``, the desktop panel, the shell's
...
def _serve_and_run_shell(host: str, port: int) -> None:
    ...
    The shell is an ordinary API client (see ``prometheus.shell``), so shell mode is just
    ``prom serve`` plus that client -- one generation path for every caller, none of it
    ...
    the chat as it streams -- ``/v1/requests`` still records every request for ``prom ctl``.
    ...
    from prometheus.launch import resolve_server_url
    from prometheus.shell.tui import run_shell
    ...
    thread = threading.Thread(target=server.run, name="prometheus-uvicorn", daemon=True)
    thread.start()
...
def run_api_server(config: ServerArgs, start_backend: Callable[[], "Any"], run_shell: bool) -> None:
    ...
    # Create/validate PROMETHEUS_API_LOG_DIR and start the writer thread up front, so a
    ...
    def _on_failure(message: str) -> None:
        ...
        logger.error("WorkerSupervisor: %s", message)
    ...
    threading.Thread(
        ...
        name="prometheus-backend-supervisor",
        daemon=True,
    ).start()
