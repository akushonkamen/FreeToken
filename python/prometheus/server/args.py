...
from prometheus.scheduler import SchedulerConfig
...
class ServerArgs(SchedulerConfig):
    ...
    # The terminal shell is attached to this server (prom shell --model / prom serve --shell-mode).
    ...
    def zmq_frontend_addr(self) -> str:
        return "ipc:///tmp/prometheus_3" + self._unique_suffix

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = "ipc:///tmp/prometheus_4" + self._unique_suffix
...
def parse_args(
    ...
    from prometheus.kvcache import SUPPORTED_CACHE_MANAGER
    from prometheus.moe import SUPPORTED_MOE_BACKENDS
    ...
    def _lazy_gpu_arg(value: str) -> tuple[str, ...]:
        from prometheus.gpu_select import gpu_arg
    ...
    parser = argparse.ArgumentParser(prog=prog, description="Prometheus Server Arguments")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(offload, or hybrid when a `prom bench bw` profile recommends it); resident "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "step's misses (perfect overlap; needs an `prom bench bw` profile, else 1). "
    ...
    # sizing flag at all, default to --moe-cache-auto so a bare `prom serve <FTW MoE>` works
