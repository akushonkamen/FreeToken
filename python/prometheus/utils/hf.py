...
def load_tokenizer(model_path: str) -> PreTrainedTokenizerBase:
    from prometheus.models.gguf.reader import gguf_config_source

    if (gguf_src := gguf_config_source(model_path)) is not None:
        from prometheus.models.gguf.tokenizer import load_gguf_tokenizer
...
def load_eos_token_ids(
    ...
    if (gguf_src := gguf_config_source(model_path)) is not None:
        from prometheus.models.gguf.tokenizer import gguf_eos_token_ids
...
def load_generation_sampling(model_path: str) -> dict[str, Any]:
    ...
    if (gguf_src := gguf_config_source(model_path)) is not None:
        from prometheus.models.gguf.reader import load_gguf_metadata
...
class RawConfigShim:
    ...
    Muse-Glimmer on transformers < 5.15). Prometheus only reads config fields
...
def _load_hf_config(model_path: str) -> Any:
    ...
    # (e.g. MiniMax-M2) refuse to load without it. Prometheus only reads config fields
...
def cached_load_hf_config(model_path: str) -> PretrainedConfig:
    ...
    if (gguf_src := gguf_config_source(model_path)) is not None:
        from prometheus.models.gguf.config import build_gguf_shim
