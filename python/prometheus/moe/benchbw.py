"""``prom bench bw``: measure host RAM vs PCIe bandwidth and pick the offload-family
...
prometheus/benchbw/<gpu-uuid>.json``) so the choice is reproducible and a multi-GPU box keeps
one profile per card.

    prom bench bw                              # per-dtype tuning bench (default: all formats)
    prom bench bw --dtype nvfp4,bf16           # only these formats
    prom bench bw --model qwen3.6-moe          # per-model detail instead
    prom bench bw --dtype all --model all      # both
    prom bench bw --gpu GPU-2f3a...            # bench a specific GPU (UUID or nvidia-smi index)
"""
...
from prometheus.gpu_select import (
    assign_gpu,
    bind_assigned_gpu,
    gpu_identity,
    single_gpu_arg,
)
from prometheus.kernel.pinned import alloc_pinned_tensor
from prometheus.moe.cpu_executor import physical_core_cpus, resolve_threads_and_affinity
from prometheus.utils import init_logger
...
# CPU MoE ISA tiers, forced via PROMETHEUS_CPU_MOE_ISA (the kernel caps down to hw/build
...
def default_out_path(gpu_uuid: str | None = None) -> str:
    ...
    from prometheus.moe.bench_profile import default_profile_path
...
def measure_cpu_moe_bw(fmt: str, wl: Workload, iters: int = 64, num_threads: int = 0,
    ...
    subset. ``isas`` (a list) additionally forces each ``PROMETHEUS_CPU_MOE_ISA`` tier over
...
def run_benchbw(
    ...
    # while the bench runs -- mirrors prom checkpoint's FTCONVERT lines. `done`/`total` count the
    # measured units (ceilings + one per benched format); `label` is what's measuring now.
    prog_on = os.environ.get("PROMETHEUS_BENCH_PROGRESS") == "1"
    ...
    exec_log = logging.getLogger("prometheus.moe.cpu_executor")
...
def main(argv: list[str] | None = None, prog: str = "prom bench bw") -> int:
    ...
    # same as prom serve --gpu: resolve, then bind by UUID at CUDA init
    try:
        assign_gpu(ns.gpu)
