...
Prometheus stores MoE experts as ModelOpt NVFP4 (packed e2m1 codes + fp8-e4m3 per-16
...
b12x        sm_120+ and CUDA>=13        ``flashinfer`` SM12x CuTe-DSL MoE (W4A16);
                                        b12x-packed weights
triton      anything (fallback)         Prometheus's own Triton kernels; the native
...
def _b12x_unusable_reason(cc: tuple[int, int]) -> str | None:
    ...
    from prometheus.kernel.backend import driver_cuda_version
...
def _b12x_min_intermediate() -> int:
    ...
    Override the threshold with ``PROMETHEUS_NVFP4_B12X_MIN_I``."""
    raw = os.environ.get("PROMETHEUS_NVFP4_B12X_MIN_I")
    if raw is None:
        return 1024
    try:
        return int(raw)
        ...
        raise ValueError(
            f"bad PROMETHEUS_NVFP4_B12X_MIN_I={raw!r}; expected an integer"
        ) from exc
...
#     i.e. it expects the merged proj as [up, gate] and emits [gate, up]. Prometheus stores
...
def b12x_repack_layer(
    ...
    *,
    ...
    for start in range(0, E, chunk):
        ...
        # so swap Prometheus's native [gate, up] halves here; swap the (ratio-folded)
...
def b12x_fused_experts(
    ...
    path must be warmed once eagerly before graph capture (Prometheus already does)."""
