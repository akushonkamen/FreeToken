"""``prom daemon <verb>`` — the client half of the daemon's own control entry point.

Kept separate from ``prom ctl`` (which targets a running *serve*): the daemon has its own dedicated
CLI. Bare ``prom daemon`` (or ``prom daemon --host/--port …``) runs the server; ``prom daemon <verb>``
...
def _build_parser(prog: str) -> argparse.ArgumentParser:
    ...
    # --timeout live on each verb (`prom daemon status --url X`), not before it.
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--url", default=os.environ.get("PROMETHEUS_DAEMON_URL", DEFAULT_URL),
                        help=f"daemon URL (default {DEFAULT_URL})")
    common.add_argument("--token", default=os.environ.get("PROMETHEUS_DAEMON_TOKEN"), help="X-FT-Token shared secret")
    common.add_argument(
        "--timeout",
        type=float,
        default=None,
    ...
    p = argparse.ArgumentParser(prog=prog, description="Control a running prom daemon")
    ...
    for name in ("start", "switch"):
        ...
        # Everything after `--` is forwarded verbatim to prom serve (opaque passthrough):
        #   prom daemon start MODEL --port 1919 -- --moe-cache-auto --graph 256
        sp.add_argument("serve_args", nargs="*", default=[], help="Extra prom serve args (after --)")
...
def main(argv: Sequence[str] | None = None, *, prog: str = "prom daemon") -> int:
