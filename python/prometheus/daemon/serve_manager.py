"""The supervisor heart: own exactly one ``prom serve`` child's lifecycle.

Spawn, kill, and orphan detection all live here, in the daemon's own process, so parent and child
are co-located and ordinary signals suffice. Torch-free: it only ever launches ``prom serve`` in a
...
def build_serve_command(
    ...
    """The serve invocation + its log path. ``python -m prometheus.cli serve`` (NOT ``-m
    prometheus``, which is a direct-server entrypoint that ignores subcommand argv) so it uses the
    daemon's own interpreter/venv with no PATH dependency."""
    argv = [python, "-m", "prometheus.cli", "serve", "--model", model, "--port", str(port), *args]
...
class ServeManager:
    ...
    def readopt(self) -> bool:
        ...
        if child is None:
            self._store.clear()
            self._emit(f"stale serve state cleared (pid={state.pid} no longer an prom serve)")
            return False
        with self._cond:
            self._child = child
