"""``prom daemon`` entrypoint: the persistent, torch-free engine supervisor.
...
def _default_state_dir() -> str:
    env = os.environ.get("PROMETHEUS_DAEMON_DIR")
    if env:
        return env
    return os.path.join(os.path.expanduser("~"), ".prometheus", "daemon")
...
def _build_parser(prog: str) -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog=prog,
        description="Prometheus engine supervisor (run the daemon server)",
        epilog=(
    ...
    p.add_argument("--token", default=os.environ.get("PROMETHEUS_DAEMON_TOKEN"), help="Optional X-FT-Token shared secret")
    p.add_argument("--default-serve-port", type=int, default=DEFAULT_SERVE_PORT, help="Port used when /engine/start omits one")
    p.add_argument("--serve-python", default=sys.executable, help="Interpreter used to launch prom serve")
...
def main(argv: Sequence[str] | None = None, *, prog: str = "prom daemon") -> int:
    ...
    try:
        ...
        print(f"prom daemon: {exc}", file=sys.stderr)
        return 1
    ...
    logger.info("prom daemon %s listening on %s:%s (state-dir=%s)", DAEMON_VERSION, args.host, args.port, state_dir)
    try:
        server.run()
    finally:
        stop_reaper.set()
