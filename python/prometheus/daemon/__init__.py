"""``prom daemon`` — the Prometheus engine supervisor (persistent, torch-free control plane).

This package MUST NOT import torch / CUDA / flashinfer / sgl_kernel / any model or kernel code,
directly or transitively. That includes ``prometheus.server.*`` (its ``__init__`` pulls
torch) and ``prometheus.utils.*`` (its ``__init__`` pulls transformers). Keep this module's imports
...
def main(argv=None, *, prog: str = "prom daemon") -> int:
    """Single entry for ``prom daemon``. A leading client verb (``prom daemon status`` …) controls a
