...
class BenchBody(BaseModel):
    # Raw `prom bench bw` args (e.g. ["--dtype", "nvfp4", "--threshold", "2.5"]); empty = all dtypes.
    args: list[str] = []
...
def _bench_profile_path(gpu_uuid: str | None) -> str | None:
    ...
    from prometheus.moe.bench_profile import default_profile_path, latest_profile_path  # torch-free
...
def _serve_gpu_uuid(args: list[str]) -> str | None:
    ...
    for i, a in enumerate(args):
        ...
        if not val:
            continue
        from prometheus.gpu_select import resolve_gpu_uuids
...
def _parse_ftbench(line: str) -> dict | None:
    """``FTBENCH <done> <total> <label>`` -> a progress dict (mirrors prom checkpoint's FTCONVERT)."""
...
def build_app(
    *,
    ...
    app = FastAPI(title="Prometheus daemon", version=DAEMON_VERSION)
    ...
    async def bench_run(body: BenchBody):
        ...
        # (mirrors /checkpoint/start). Runs `prom bench bw` on the engine HOST (so the profile lands
        ...
        # is the raw arg list, so any `prom bench bw` flag (--dtype/--model/--threshold/...) passes
        # through. torch stays out of the daemon (child process), which also frees VRAM on exit.
        await run(lifecycle_pool, manager.stop)

        async def gen():
            env = {**os.environ, "PROMETHEUS_BENCH_PROGRESS": "1"}
            argv = [sys.executable, "-m", "prometheus.cli", "bench", "bw", *body.args]
            try:
