# `ft daemon` — a simple daemon mode for Prometheus
...
`prometheus.server.*` (its `__init__` pulls torch) or `prometheus.utils.*` (its `__init__` pulls
...
`--state-dir` (default `~/.prometheus/daemon`, override with `$PROMETHEUS_DAEMON_DIR`).
...
Target a non-default daemon with `--url http://host:1900` (or `$PROMETHEUS_DAEMON_URL`) and
`--token`/`$PROMETHEUS_DAEMON_TOKEN`.
...
Set `--token` (or `$PROMETHEUS_DAEMON_TOKEN`) to require an `X-FT-Token` header on everything
except `/health`.
