...
from prometheus.kvcache import create_kv_pool, resolve_pool_class
from prometheus.kvcache.base import CacheRebuildRejected
from prometheus.kvcache.cache_status import _supports_swa_ratio
...
def _flashinfer_available() -> bool:
    from prometheus.kernel.backend import is_flashinfer_installed
...
def _validate_attention_backend_choice(config, override, required: frozenset[AttnType]) -> None:
    ...
    from prometheus.attention import validate_attn_backend
    ...
    for part in backend_parts:
        ...
        if info.requires_flashinfer and not _flashinfer_available():
            raise RuntimeError(
                ...
                "not installed. Install it with `pip install 'prometheus[fi]'` (or "
                "'prometheus[accel]'), or use --attention-backend triton."
            )
        if info.requires_sgl_kernel and not _sgl_flash_attn_available():
            raise RuntimeError(
                ...
                "not installed. Install it with `pip install 'prometheus[sgl]'` (or "
                "'prometheus[accel]'), or use --attention-backend triton."
            )
...
class Engine:
    ...
    def _resolve_auto_moe_cache_size(self, config: EngineConfig, banks) -> tuple[int, int, bool]:
        ...
        """
        from prometheus.engine.cache_budget import expert_bytes_per_slot, resolve_moe_cache_auto
    ...
    def _init_offload_moe_cache(self, config: EngineConfig) -> OffloadMoeCache:
        ...
        cache.collect_stats = config.moe_collect_stats or bool(os.getenv("PROM_MOE_STATS"))
    ...
    def _resolve_hybrid_fetch(self, config: EngineConfig, cache) -> None:
        ...
        achieved bandwidths come from the cached `prom bench bw` profile (the same one the
        ...
        from prometheus.moe.bench_profile import load_hybrid_fetch_fraction
        ...
        if fraction is None:
            ...
            logger.warning_rank0(
                "--moe-hybrid-max-fetch auto: no usable `prom bench bw` profile for "
    ...
    def _init_cpu_moe_executor(self, config: EngineConfig, cache, layers) -> None:
        ...
        """
        from prometheus.moe.cpu_executor import CpuMoeExecutor
    ...
    def _target_moe_and_expert_bytes(self, moe_cache_size: int | None) -> tuple[int, int]:
        from prometheus.engine.cache_budget import expert_bytes_per_slot
...
def _adjust_config(config: EngineConfig):
    ...
    if is_moe and config.moe_backend == "auto":
        ...
        # Hardware-adaptive config: a cached `prom bench bw` profile can upgrade
        ...
        from prometheus.moe.bench_profile import load_backend_recommendation

        gpu_name, gpu_uuid = _profile_gpu()
        if load_backend_recommendation(bench_fmt, gpu_name=gpu_name, gpu_uuid=gpu_uuid) == "hybrid":
            from prometheus.moe.cpu_executor import compiled_extension_supports
        ...
        if (
            ...
            # backend is already offload-family at *parse* time. A bare `prom serve <FTW MoE
