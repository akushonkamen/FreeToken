...
# default. Set PROMETHEUS_HYBRID_OVERLAP=0 to force the serial path (CPU sync before the
# GPU work) -- a measurement-only escape hatch to A/B the overlap benefit.
_HYBRID_OVERLAP = os.getenv("PROMETHEUS_HYBRID_OVERLAP", "1") != "0"
...
class MoELayer(BaseOP):
    ...
    def _alloc_resident_experts(self, intermediate_size_per_partition: int) -> None:
        ...
        if self.weight_format == "fp8_block":
            ...
            from prometheus.kernel.triton.fp8_block_linear import FP8
    ...
    def _resident_gemm(
        ...
        if self.weight_format == "fp8_block":
            ...
            from prometheus.moe.fused_fp8_block import (
...
class OffloadMoELayer(MoELayer):
    ...
    def _decode_routed(
        ...
        cache.ensure_experts(
            self.layer_id,
            topk_ids,
            verify=bool(getattr(get_global_ctx().batch, "spec_verify", False)),
        )
        cache.copy_missing()
    ...
    def _decode_hybrid(
        ...
        # Measurement knob: PROMETHEUS_HYBRID_OVERLAP=0 syncs the CPU pool *before* the
    ...
    def _expert_gemm(
        ...
        *,
        ...
        if fmt in ("nvfp4_marlin", "nvfp4_b12x"):
            ...
            from prometheus.moe.nvfp4_backends import b12x_fused_experts, marlin_fused_experts
        ...
        if fmt == "nvfp4":
            # Prometheus's Triton inline-dequant kernels over the native ModelOpt
            ...
            if is_prefill:
                from prometheus.moe.fused_nvfp4 import fused_experts_nvfp4
            ...
            # shapes, no host sync).
            from prometheus.moe.fused_nvfp4 import fused_experts_decode_nvfp4_marlin
        ...
        if fmt == "q4_0":
            ...
            from prometheus.moe.fused_q4_0 import fused_experts_gguf_q4_0
        ...
        if fmt == "mxfp4_triton":
            ...
            from prometheus.moe.fused_mxfp4 import (
        ...
        if fmt == "ds_fp4":
            ...
            if is_prefill and n is not None:
                from prometheus.moe.fused_ds_fp4 import routed_experts_fp4_prefill
            ...
            from prometheus.moe.fused_ds_fp4 import routed_experts_fp4
