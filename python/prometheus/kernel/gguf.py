...
separate, torch-native extension that sits alongside Prometheus's tvm-ffi kernels.
...
def _host_compiler() -> str | None:
    ...
    to an older gcc. Override with ``PROMETHEUS_GGUF_HOST_CXX``.
    """
    override = os.environ.get("PROMETHEUS_GGUF_HOST_CXX")
    if override:
        return override
