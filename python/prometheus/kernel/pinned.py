...
def _load_pinned_extension():
    try:
        return importlib.import_module("prometheus.kernel._pinned_tensor")
    except ImportError as exc:
        raise ImportError(
            "prometheus.kernel._pinned_tensor is not installed. Reinstall Prometheus "
...
def _host_ptr_identity() -> bool:
    # cached per process: Prometheus pins one CUDA device per process (set at engine launch)
