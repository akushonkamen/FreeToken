...
KERNEL_CACHE_PACKAGE = "prometheus_kernel_cache"
KERNEL_CACHE_DIR_ENV = "PROMETHEUS_KERNEL_CACHE_DIR"
DISABLE_KERNEL_CACHE_ENV = "PROMETHEUS_DISABLE_KERNEL_CACHE"
DISABLE_KERNEL_CACHE_VERSION_CHECK_ENV = "PROMETHEUS_DISABLE_KERNEL_CACHE_VERSION_CHECK"
DISABLE_JIT_ENV = "PROMETHEUS_DISABLE_JIT"
...
def _make_name(*args: str) -> str:
    return "prometheus__" + "_".join(str(arg) for arg in args)
...
def _kernel_cache_dir() -> pathlib.Path | None:
    ...
    runtime_version = _prometheus_version()
    if not _env_enabled(DISABLE_KERNEL_CACHE_VERSION_CHECK_ENV):
        if not _kernel_cache_version_ok(package_version, runtime_version):
            raise RuntimeError(
                "prometheus-kernel-cache version "
                f"{package_version!r} does not match prometheus version {runtime_version!r}"
            )
        cache_cuda = re.search(r"\+cu(\d{2,})", package_version)
        if cache_cuda is not None:
            from prometheus.kernel._toolchain import torch_cuda_major

            cache_major = int(cache_cuda.group(1)[:-1])
            torch_major = torch_cuda_major()
            if torch_major is not None and cache_major != torch_major:
                raise RuntimeError(
                    f"prometheus-kernel-cache {package_version!r} was built for CUDA "
...
def _load_prebuilt(name: str) -> Module | None:
    ...
    if cache_dir is None:
        if _env_enabled(DISABLE_JIT_ENV):
            raise RuntimeError(
                "JIT compilation is disabled by PROMETHEUS_DISABLE_JIT, "
...
def load_aot(
    *args: str,
    ...
    if cuda_files:
        from prometheus.kernel._toolchain import check_nvcc_matches_torch
