...
experts. Prometheus used to *dequantize these to bf16 at load* and run cuBLAS bf16 GEMV -- which,
...
from prometheus.layers import BaseOP
from prometheus.layers.base import _concat_prefix

from prometheus.kernel.triton.e4m3_compat import (
    e4m3_kernel_view,
    e4m3_native_cx,
    e4m3_u8_to_f16_x128,
    e4m3_u8_to_f32,
...
# Escape hatch: PROMETHEUS_DEBUG_DENSE_NVFP4_REF=1 swaps the triton kernels for a dequant_nvfp4 +
# torch matmul reference (numeric A-B debugging). Evaluated once; the kernels are the default.
_USE_REF = os.environ.get("PROMETHEUS_DEBUG_DENSE_NVFP4_REF") == "1"
...
def _ref(a: torch.Tensor, packed: torch.Tensor, scale: torch.Tensor,
    ...
    then a plain matmul. Used by PROMETHEUS_DEBUG_DENSE_NVFP4_REF=1 for A-B validation."""
