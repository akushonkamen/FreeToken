...
``PROMETHEUS_FORCE_E4M3_EMU=1`` (or true/yes/on) forces the emulated path on any
...
def _env_force() -> bool:
    return os.environ.get("PROMETHEUS_FORCE_E4M3_EMU", "").lower() in ("1", "true", "yes", "on")
...
def e4m3_native() -> bool:
    ...
    if _env_force() != FORCE_EMU:
        raise RuntimeError(
            "PROMETHEUS_FORCE_E4M3_EMU changed after import: the flag is read once at "
    ...
    if _native is None:
        if FORCE_EMU:
            ...
            from prometheus.gpu_select import assigned_visible_gpu
