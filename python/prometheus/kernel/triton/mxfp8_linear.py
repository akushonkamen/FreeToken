...
from prometheus.layers.base import BaseOP

from prometheus.kernel.triton.e4m3_compat import e4m3_kernel_view, e4m3_native_cx, e4m3_u8_to_f32

FP8 = torch.float8_e4m3fn
MXFP8_BLOCK = 32
_TL_DTYPE = {torch.bfloat16: tl.bfloat16, torch.float16: tl.float16, torch.float32: tl.float32}

# Escape hatch: PROMETHEUS_DEBUG_MXFP8_REF=1 swaps the kernels for a pure-torch
# dequant matmul (numeric reference / A-B debugging). Evaluated once, logged once.
_USE_REF = os.environ.get("PROMETHEUS_DEBUG_MXFP8_REF") == "1"
if _USE_REF:
    from prometheus.utils import init_logger

    init_logger(__name__).info(
        "PROMETHEUS_DEBUG_MXFP8_REF=1: MXFP8 linears serve the pure-torch fp32 "
