...
# equivalents and the CPU/NPU branches are dropped (Prometheus is NVIDIA-only). Only the
# forward path is kept. Public entry: ``rms_norm_gated`` -- fused RMSNorm(x) * silu(z) in
# one triton kernel, replacing Prometheus's pure-PyTorch _GatedRMSNorm (~8 elementwise
