...
spec name, so the shape lists in :mod:`prometheus.kernel.aot` must cover every
(model, weight-format) pair Prometheus serves. Mirroring flashinfer's ``aot``
...
# prebuilt so the suite also runs under PROMETHEUS_DISABLE_JIT=1 release validation.
