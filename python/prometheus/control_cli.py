...
def _format_cache_status(doc: dict[str, Any]) -> str:
    """The pool table (shared with the shell's ``/cache`` -- see prometheus.cache_report), plus
    the one thing only this CLI reports: the outcome of the last rebuild."""
    from prometheus.cache_report import format_cache_status
...
def _build_parser(prog: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=f"Prometheus server URL (default: {DEFAULT_BASE_URL})",
    )
...
def main(argv: Sequence[str] | None = None, *, prog: str = "prom ctl") -> int:
...
def _rebuild_body(targets: dict[str, int], geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    from prometheus.cache_report import CachePools, pages_for_tokens
...
def _run_cache_rebuild(args: argparse.Namespace) -> int:
    ...
    # the sizes itself. Same answer `prom ctl cache status` gives, and the same table the shell's
