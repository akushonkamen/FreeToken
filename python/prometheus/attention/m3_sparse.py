...
def _pick_inner_backend(block_size: int) -> str:
    ...
    ``PROMETHEUS_M3_INNER_BACKEND`` overrides (e.g. "triton" on a box with a
    ...
    override = os.getenv("PROMETHEUS_M3_INNER_BACKEND")
    if override:
        for part in override.split(","):
            try:
                ...
                raise ValueError(
                    f"PROMETHEUS_M3_INNER_BACKEND={override!r}: unknown attention "
                    f"backend {part!r}"
                ) from None
            if AttnType.FULL not in info.supported_types:
                raise ValueError(
                    f"PROMETHEUS_M3_INNER_BACKEND={override!r}: {part!r} does not "
                    "serve FULL attention (the dense leading layers)."
                )
        if not _page_ok(override):
            raise ValueError(
                f"PROMETHEUS_M3_INNER_BACKEND={override!r} cannot address the "
                f"{block_size}-token pages this model's pool uses."
            )
        logger.info(f"m3_sparse dense-layer backend: {override} (env override)")
        return override

    from prometheus.engine.engine import _resolve_auto_attention_backend
    ...
    if not _page_ok(name):
        ...
        # page filter applied.
        from prometheus.engine.engine import _backend_requirements_met
        from prometheus.utils.arch import is_sm90_family
...
class M3SparseAttnBackend(BaseAttnBackend):
    def __init__(self, config: ModelConfig) -> None:
        ...
        from prometheus.attention import create_attention_backend
    ...
    def _decode(self, md, layer_id: int, slot: int, q, index_q) -> torch.Tensor:
        from prometheus.kernel.triton.minimax_m3_sparse import (
            m3_index_decode,
