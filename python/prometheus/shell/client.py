"""The Prometheus shell's view of a running server: an ordinary API client.
...
``prom shell`` can attach to any server, in this process or on another machine.

Transport follows what the repo already does: the ``openai`` SDK for generation (like
``prometheus.benchmark.client``), stdlib ``urllib`` for the control-plane JSON (like ``prom ctl``
...
LOCAL_API_KEY = "prometheus-local"
    ...
    async def chat(
        ...
        *,
        ...
        if sampling.top_k is not None:
            extra_body["top_k"] = sampling.top_k  # not an OpenAI field; Prometheus accepts it
        ...
        try:
            async for chunk in stream:
                ...
                for choice in chunk.choices or []:
                    ...
                    if delta is None:
                        continue
                    # reasoning_content is Prometheus's (and vLLM/SGLang's) split channel; the
