"""``prom shell`` -- an interactive terminal chat that drives a Prometheus server over its API.

Two ways in, one code path:

* ``prom shell`` attaches to a server that is already running (``--server``, ``$PROMETHEUS_HOST``,
  else ``http://127.0.0.1:1919``), exactly like ``prom launch`` attaches an agent. Nothing is
  loaded locally -- no torch import, no GPU -- so it works against a remote box too.
* ``prom shell --model <path> [engine flags]`` starts the engine here first (the same thing
  ``prom serve --shell-mode`` does) and then attaches to it over the loopback.
...
def _build_parser(prog: str) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=prog,
        description="Chat with a running Prometheus server in the terminal.",
        epilog=(
            "Pass --model <path> (plus any prom serve flag) to start an engine here instead of "
            "attaching to one."
        ),
    )
    parser.add_argument(
        ...
        dest="server",
        default=None,
        help="Prometheus server URL (default: $PROMETHEUS_HOST, else http://127.0.0.1:1919)",
    )
    return parser


def main(argv: Sequence[str] | None = None, *, prog: str = "prom shell") -> int:
