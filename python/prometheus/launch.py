...
CODEX_PROFILE = "prometheus-launch"
CODEX_PROVIDER_NAME = "Prometheus"
CODEX_CATALOG_NAME = "prometheus-model.json"
CODEX_PROVIDER_API_KEY_ENV = "PROMETHEUS_API_KEY"
...
OPENCODE_PROVIDER = "prometheus"
OPENCODE_PROVIDER_NAME = "Prometheus"
OPENCLAW_PROVIDER = "prometheus"
OPENCLAW_PROVIDER_NAME = "Prometheus"
HERMES_API_KEY = "prometheus-local"
HERMES_MIN_CONTEXT_LENGTH = 64_000  # Hermes refuses to start below this
DSH_API_KEY = "prometheus"
DSH_MIN_NODE = (22, 19)  # dsh's floor; its npm package declares no engines gate
DSH_LAUNCH_SETTINGS_NAME = "prometheus-launch.settings.yaml"
DSH_LAUNCH_PATCH_NAME = "prometheus-launch.cordis.patch.yml"
...
def resolve_server_url(server: str | None) -> ServerURL:
    raw = server or os.environ.get("PROMETHEUS_HOST") or DEFAULT_SERVER
    raw = raw.strip()
    if not raw:
        raw = DEFAULT_SERVER
    if "://" not in raw:
    ...
    if not parsed.hostname:
        raise ValueError(f"Invalid Prometheus server URL: {server!r}")

    path = parsed.path.rstrip("/")
    if path not in ("", "/v1"):
        raise ValueError(
            "Prometheus launch --server accepts only an origin or /v1 base URL"
        )
...
def discover_server_model(server: ServerURL) -> ServedModel:
    try:
        ...
        raise RuntimeError(f"Cannot connect to Prometheus server at {server.origin}: {exc}") from exc

    if not isinstance(payload, dict):
        raise RuntimeError("Prometheus server returned an invalid /v1/models response")
    ...
    if not models:
        raise RuntimeError(f"Prometheus server at {server.origin} reported no models")
...
def prepare_codex(ctx: LaunchContext) -> CommandSpec:
    ...
    return CommandSpec(
        argv=argv,
        env={CODEX_PROVIDER_API_KEY_ENV: "prometheus"},
...
def prepare_claude(ctx: LaunchContext) -> CommandSpec:
    ...
    return CommandSpec(
        ...
        env={
            ...
            "ANTHROPIC_API_KEY": "",
            "ANTHROPIC_AUTH_TOKEN": "prometheus",
...
def _openclaw_has_prometheus_provider(config: dict[str, object]) -> bool:
...
def _confirm_openclaw_first_patch(ctx: LaunchContext, config_path: Path) -> bool:
    print(
        "Prometheus launch will modify OpenClaw config for the first time.",
        file=sys.stderr,
    )
...
def _patch_openclaw_config(
    ...
    provider["apiKey"] = "prometheus-local"
...
def prepare_openclaw(ctx: LaunchContext) -> CommandSpec:
    if not ctx.dry_run:
        ...
        if not _openclaw_has_prometheus_provider(config):
...
def prepare_hermes(ctx: LaunchContext) -> CommandSpec:
    ...
    patch only the model section (preserving the user's other config) to point at Prometheus.
    ...
    if not ctx.dry_run:
        ...
        # A non-empty dummy: Prometheus is unauthenticated, but some clients reject an empty key.
...
def prepare_dsh(ctx: LaunchContext) -> CommandSpec:
    ...
    ``prometheus-launch.settings.yaml`` for this invocation only, so the user's
    ...
    dummy DEEPSEEK_API_KEY satisfies dsh's non-empty key requirement; Prometheus
...
def parse_argv(argv: list[str], prog: str = "python -m prometheus.launch") -> argparse.Namespace:
    if "--" in argv:
    ...
    parser = argparse.ArgumentParser(
        prog=prog,
        description="Configure and launch a coding agent against an existing Prometheus server.",
    )
    parser.add_argument("agent", choices=sorted(PREPARERS))
    parser.add_argument("--server", help="Prometheus server origin or /v1 URL")
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def print_dry_run(ctx: LaunchContext, spec: CommandSpec) -> None:
    print(f"Prometheus server: {ctx.server.origin}")
...
def main(argv: list[str] | None = None, prog: str = "python -m prometheus.launch") -> int:
