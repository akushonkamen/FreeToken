...
from prometheus.kvcache import BaseCacheHandle, MatchResult, create_prefix_cache
from prometheus.utils import align_down, div_ceil
...
def _swa_eviction_interval() -> int:
    raw = os.environ.get("PROMETHEUS_SWA_EVICTION_INTERVAL", "128")
    try:
        return max(1, int(raw))
    except ValueError:
        raise ValueError(f"PROMETHEUS_SWA_EVICTION_INTERVAL must be an integer, got {raw!r}")
...
class CacheManager:
    ...
    def _make_prefix_cache(self, device, page_size, type):
        if type == "hybrid_radix":
            from prometheus.kvcache.hybrid_radix_cache import HybridRadixCache
            return HybridRadixCache(device, page_size)
        if type == "swa_radix":
            from prometheus.kvcache.swa_radix_cache import SWARadixCache
    ...
    def match_req(self, req: PendingReq) -> MatchResult:
        ...
        if self.is_swa:
            from prometheus.kvcache.swa_radix_cache import SWACacheHandle
            m = self.prefix_cache.match_prefix(ids)
            return MatchResult(SWACacheHandle(m.cached_len, m.node, m.kv_indices))
        if self.is_hybrid:
            from prometheus.kvcache.hybrid_radix_cache import HybridCacheHandle
            m = self.prefix_cache.match_prefix(ids)
            return MatchResult(
