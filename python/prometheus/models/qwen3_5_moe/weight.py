...
def _rename(raw_name: str) -> str | None:
    ...
    if raw_name.startswith(("mtp.", "model.visual.", "visual.")):
        return None
    # ModelOpt FP8 KV-cache static scales (full-attention layers only). Prometheus keeps the
...
def setup_offload_expert_banks(
    ...
    demand in ``_expert_gemm``). ``PROMETHEUS_FP8_EXPERTS=bf16`` instead dequantizes every expert
    ...
    if eq != "fp8_block":
        from prometheus.moe.expert_banks import _PROVIDERS  # nvfp4 -> _nvfp4_banks, none -> _bf16_banks
    ...
    from prometheus.moe.expert_banks import ExpertBanks

    mode = os.environ.get("PROMETHEUS_FP8_EXPERTS", "fp8").strip().lower()
    if mode not in ("fp8", "bf16"):
        raise ValueError(f"PROMETHEUS_FP8_EXPERTS must be 'fp8' or 'bf16', got {mode!r}")
...
def _build_fp8_expert_banks(
    ...
    from prometheus.models.weight import experts_scattered, iter_expert_tensors_parallel
    ...
    if dummy:
        ...
        if hb is not None and torch.cuda.is_available():
            from prometheus.moe.host_banks import pin_banks
...
def _setup_bf16_dequant_banks(model_path, model_config, device, dummy: bool, *, layer_sink=None):
    from prometheus.models.weight import dummy_moe_expert_sources
    from prometheus.moe.expert_banks import ExpertBanks
    ...
    from prometheus.kernel.triton.fp8_block_linear import dequant_block_fp8
    from prometheus.moe.host_banks import LayerCompletionTracker, PinPipeline, alloc_layer_banks
