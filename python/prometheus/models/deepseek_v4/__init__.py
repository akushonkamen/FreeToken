"""DeepSeek-V4-Flash support for Prometheus.
...
Prometheus's primitives. The exotic ops are reimplemented as Triton kernels (see
``prometheus/kernel/triton/dsv4_*.py``); the routed FP4 experts are served from
Prometheus's :class:`~prometheus.moe.offload_cache.OffloadMoeCache` so only a subset
...
:mod:`prometheus.attention.dsv4_sparse` and :mod:`prometheus.kvcache.dsv4_paged_pool`).
"""
