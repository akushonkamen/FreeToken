"""DeepSeek-V4-Flash model (engine-native Prometheus port of inference/model.py).
...
sqrtsoftplus / hash MoE), wired onto Prometheus's shared paged engine:

  - KV lives in DSV4-owned paged pools (:class:`~prometheus.kvcache.dsv4_paged_pool.DSV4PagedKVCache`):
...
Heavy ops are Prometheus Triton kernels. Precision matches the reference (FP8/FP4
...
from prometheus.kernel.triton.dsv4.hc import hc_post_combine, hc_pre_combine
from prometheus.kernel.triton.dsv4.sinkhorn import hc_split_sinkhorn
