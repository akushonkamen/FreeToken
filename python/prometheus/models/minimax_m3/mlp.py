...
(``ModelConfig.dense_quant == "mxfp8"``, from PROMETHEUS_M3_MLP_MXFP8) -- decode reads
every shared expert each token, so this halves the resident-MLP weight traffic and
frees the same VRAM for expert-cache slots. ``PROMETHEUS_M3_MLP_MXFP8=0`` dequantizes
...
from prometheus.layers import BaseOP, LinearReplicated, swigluoai_and_mul
...
def make_proj(quant: str, in_features: int, out_features: int) -> BaseOP:
    ...
    if quant == "mxfp8":
        from prometheus.kernel.triton.mxfp8_linear import Mxfp8Linear
