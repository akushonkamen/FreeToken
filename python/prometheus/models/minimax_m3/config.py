...
lives in ``text_config`` and the weights carry a ``language_model.`` prefix. Prometheus
...
layers and the sparse layers. ``PROMETHEUS_M3_SPARSE=0`` zeroes the index dims (dense
...
Resident-weight quantization is resolved HERE (from the PROMETHEUS_M3_*_MXFP8 env
...
def parse_config(hf_config: Any) -> ModelConfig:
    ...
    # Dev/testing only: cap the layer count (e.g. PROMETHEUS_M3_MAX_LAYERS=5 -> 3 dense
    ...
    _cap = os.environ.get("PROMETHEUS_M3_MAX_LAYERS")
    if _cap and int(_cap) < num_layers:  # a cap >= the real count changes nothing
        num_layers = int(_cap)
        _log_mode_once(
            f"cap={num_layers}",
            f"PROMETHEUS_M3_MAX_LAYERS: serving a TRUNCATED model "
            f"({num_layers}/{text.num_hidden_layers} layers) -- dev/testing only, "
            "outputs are garbage; unset the env for real serving.",
        )

    sparse_enabled = os.getenv("PROMETHEUS_M3_SPARSE", "1") != "0"
    if not sparse_enabled:
        _log_mode_once(
            "sparse=0",
            "PROMETHEUS_M3_SPARSE=0: serving the DENSE-attention ablation (plain "
    ...
    attn_mxfp8 = os.getenv("PROMETHEUS_M3_ATTN_MXFP8", "1") != "0"
    mlp_mxfp8 = os.getenv("PROMETHEUS_M3_MLP_MXFP8", "1") != "0"
    _log_mode_once(
        ...
        "(PROMETHEUS_M3_ATTN_MXFP8/PROMETHEUS_M3_MLP_MXFP8; an FTW checkpoint "
    ...
    return ModelConfig(
        ...
        # never disagree. PROMETHEUS_M3_SPARSE=0 zeroes the index dims (dense ablation:
