...
Resident-weight quantization is resolved HERE (from the PROMETHEUS_GLM_*_FP8 env
...
_ATTN_FP8 = os.getenv("PROMETHEUS_GLM_ATTN_FP8", "1") != "0"
_MLP_FP8 = os.getenv("PROMETHEUS_GLM_MLP_FP8", "1") != "0"
...
def _dsa_on(args, num_layers: int) -> bool:
    ...
    return (
        ...
        and os.getenv("PROMETHEUS_GLM_DSA", "1") != "0"
    )
...
def parse_config(hf_config: Any, model_path: str | None = None) -> ModelConfig:
    ...
    # Dev/testing only: cap the layer count (e.g. PROMETHEUS_GLM_DSA_MAX_LAYERS=5 -> 3 dense
    ...
    _cap = os.environ.get("PROMETHEUS_GLM_DSA_MAX_LAYERS")
    if _cap:
    ...
    return ModelConfig(
        ...
        # so they can never disagree. PROMETHEUS_GLM_DSA=0 zeroes them (dense ablation:
