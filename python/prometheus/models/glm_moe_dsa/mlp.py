...
PROMETHEUS_GLM_MLP_FP8): decode reads every shared expert each token, so this halves
~5.6 GiB/token of weight traffic and frees the same VRAM for expert-cache slots.
``PROMETHEUS_GLM_MLP_FP8=0`` restores the checkpoint-faithful bf16 weights.
"""
