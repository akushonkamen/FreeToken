"""Gemma 4 GGUF adapter: build the Prometheus ``ModelConfig`` from GGUF metadata.
...
from prometheus.models.gguf.dequant import GGML_Q4_0, GGML_Q6_K, dequantize, row_bytes

if TYPE_CHECKING:
    from prometheus.models.gguf.config import GgufConfigShim
...
def _full_rotary_dim(shim: "GgufConfigShim", full_head_dim: int) -> int:
    ...
    """
    from prometheus.models.gguf.reader import iter_gguf_tensors
...
# --------------------------------------------------------------------------------------
# Weight loading: GGUF tensor names -> Prometheus gemma4 module params.
# --------------------------------------------------------------------------------------

# Per-layer 1:1 norm/scale tensors (gguf suffix -> prometheus module-relative name). All
...
def convert_gemma4_to_gguf(model, config: ModelConfig) -> None:
    ...
    """
    from prometheus.layers.gguf import GGUFEmbedding, GGUFLinear
...
def dummy_q4_0_expert_sources(config: ModelConfig) -> dict[str, list[torch.Tensor]]:
    ...
    from prometheus.moe.host_banks import alloc_layer_banks, pin_banks
