...
block-FP8, per-tensor-FP8 and NVFP4 implementations live under ``prometheus.kernel.triton``;
the bf16 fallback is the framework's TP-aware ``prometheus.layers``. Only the *dispatch*
...
def make_col_merged_quant(expert_quant: str, attn_quant: str, in_f: int,
    ...
    from prometheus.layers import LinearColParallelMerged
...
def make_replicated_quant(expert_quant: str, attn_quant: str, in_f: int, out_f: int,
    ...
    if expert_quant == "fp8_block":
        from prometheus.kernel.triton.fp8_block_linear import Fp8BlockLinear

        return Fp8BlockLinear(in_f, out_f, has_bias)
    if attn_quant == "fp8_pertensor":
        from prometheus.kernel.triton.fp8_pertensor_linear import Fp8PerTensorLinear

        return Fp8PerTensorLinear(in_f, out_f, has_bias)
    if attn_quant == "nvfp4":  # compressed-tensors W4A16 attention o_proj / GDN out_proj
        from prometheus.kernel.triton.nvfp4_linear import Nvfp4DenseLinear

        return Nvfp4DenseLinear(in_f, out_f, has_bias)
    from prometheus.layers import LinearReplicated
