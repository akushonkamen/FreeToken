...
def vision_load_enabled() -> bool:
    ...
    skip building and loading them unless ``PROMETHEUS_LOAD_VISION=1`` is set."""
    return os.getenv("PROMETHEUS_LOAD_VISION", "0").strip().lower() in _VISION_TRUE
...
def detect_compressed_tensors_nvfp4(hf_config: Any) -> bool:
    ...
    for g in (groups.values() if isinstance(groups, dict) else []):
        ...
        raise ValueError(
            ...
            f"(group_size={group_size}, strategy={strategy!r}); Prometheus serves "
