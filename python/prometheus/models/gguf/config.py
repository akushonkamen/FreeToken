...
``metadata`` to build the Prometheus ``ModelConfig``.
"""
...
# GGUF ``general.architecture`` -> Prometheus registry key (a GGUF-specific spec that
    ...
    if names:
        ...
        if present is None:
            raise ValueError(
                ...
                "reconvert the checkpoint with the current prometheus.checkpoint.convert"
            )
