"""Convert an HF safetensors checkpoint into a Prometheus Weight (FTW) checkpoint.
...
# PROMETHEUS_CONVERT_PROGRESS=1 so plain CLI use isn't spammed; the human tqdm bars stay on
# stderr. Phases: `dense` (indeterminate, done=total=0), `experts` (byte totals), `finalize`.
_EMIT_PROGRESS = os.environ.get("PROMETHEUS_CONVERT_PROGRESS") == "1"
...
# .prometheus_expert_cache: the legacy per-bank cache (can be tens of GB of stale .bin)
_SKIP_DIRS = (".git", ".cache", ".prometheus_expert_cache")
...
def _copy_metadata(model_path: str, out_dir: str) -> list[str]:
    ...
    if os.path.isfile(model_path):
        ...
        from prometheus.models.gguf.reader import (
            FTW_METADATA_GGUF,
            is_gguf_path,
            write_metadata_gguf,
        )
    ...
    def __call__(self, layer_id: int, banks: dict) -> None:
        with self._lock:
            ...
            if self._bar is None:
                from prometheus.utils.progress import byte_bar
...
def convert_checkpoint(
    ...
    *,
    ...
    from prometheus.distributed import DistributedInfo, set_tp_info, try_get_tp_info
    from prometheus.engine.config import EngineConfig
    from prometheus.models.weight import load_weight
    ...
    from prometheus.utils.progress import byte_bar, count_bar
