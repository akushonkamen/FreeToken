"""Prometheus Weight (FTW) checkpoint: one O_DIRECT-friendly on-disk format for a whole model.
...
Layout on disk::
    ...
    <dir>/prometheus_weight.json        # index: tensors[] + shards[] + meta
    <dir>/prometheus-00000.ftw         # the byte region, sliced <= shard_limit
    <dir>/prometheus-00001.ftw
...
def is_ftw_checkpoint(path: str) -> bool:
    """True if ``path`` is a directory holding a Prometheus Weight (FTW) index."""
...
def load_ftw_banks(
    ...
    * **Flat region** (pre-existing files, and non-streamable formats): one entry per
      ...
      head-offset slice.
    * **Per-layer** (streamable-format conversion, see :mod:`prometheus.checkpoint.convert`):
    ...
    """
    from prometheus.moe.host_banks import HostBank, PinPipeline, alloc_banks
    from prometheus.utils.progress import byte_bar
