...
| Command | Purpose |
|---|---|
| `prom serve` | Start the API server (OpenAI `/v1/*`, Anthropic `/v1/messages`, Responses) |
| `prom shell` | Chat with a server in the terminal |
| `prom ctl` | Query and manage a running server over HTTP |
| `prom launch` | Configure and launch a coding agent against a server |
| `prom checkpoint` | Convert an HF checkpoint to the FTW fast-load format |
| `prom bench bw` | Benchmark CPU vs PCIe bandwidth to calibrate the MoE backend |
...
```bash
prom serve --model <path-or-hf-id> [options]
```
...
| Flag | Default | Meaning |
|---|---|---|
| `--moe-backend` | auto | `fused`/`offload`/`cpu`/`hybrid`; auto → offload, or hybrid with a `prom bench bw` profile |
...
```bash
prom shell                                    # attach to a running server
prom shell --model ~/models/Qwen3.6-35B-A3B   # serve + chat in one process
```
...
```bash
prom ctl [--base-url http://127.0.0.1:1919] [--timeout 10] [--json] <subcommand>
```
...
```bash
prom launch {claude,codex,dsh,hermes,openclaw,opencode} [options] [-- <agent args>]
```
...
```bash
prom checkpoint --model <hf_dir> --out <ftw_dir> [--dtype bfloat16] [--moe-backend offload] [--shard-gib 8] [--device cuda:0]
```

Converts an HF safetensors checkpoint to FTW, Prometheus's self-contained
fast-load format; point `prom serve --model` at the output dir. `--moe-backend
...
```bash
prom bench bw                       # once per machine
prom bench bw --dtype nvfp4,bf16    # only the formats you serve
```

Measures host-RAM vs PCIe bandwidth with the real cpu/offload MoE kernels and
writes a profile (`~/.cache/prometheus/benchbw.json`) that `prom serve
