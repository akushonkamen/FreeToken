...
Assumes Prometheus is installed — see [install.md](install.md).

## Launch a server

```bash
prom serve --model ~/models/Qwen3.6-35B-A3B
```
...
Prometheus serves the OpenAI API (`/v1/chat/completions`, `/v1/responses`,
...
```bash
prom shell                                    # attach to the server above
prom shell --model ~/models/Qwen3.6-35B-A3B   # start an engine and chat, one process
```
...
```bash
prom launch claude   # claude / codex / dsh / hermes / openclaw / opencode
```
