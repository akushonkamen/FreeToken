...
Prometheus loads HF safetensors checkpoints directly (plus native GGUF for
...
`prom serve --moe-backend {auto,fused,offload,cpu,hybrid}`:
...
- **hybrid** — per step, fetches some misses over PCIe and computes the rest on
  CPU, overlapped. Run `prom bench bw` once per machine to calibrate the split.
- **auto** — dense models always resolve to `fused`; MoE models resolve to
  `offload`, upgraded to `hybrid` when a cached `prom bench bw` profile
  recommends it.

## Notes

- `prom checkpoint` conversion is optional — it pre-converts a checkpoint into
  Prometheus's fast-load format, and `prom serve --model` auto-detects the result.
